...
class GradientSlider: UIControl {
    ...
    var delegate: GradientSliderDelegate?
    
    @IBInspectable
    var currentValue: CGFloat = 0.5 {
        didSet {
            currentValue = max(minValue, min(currentValue, maxValue))
            updatePaths()
        }
    }
    ...
    @IBInspectable
    var secondColor: UIColor = .systemYellow {
        didSet {
            updatePaths()
        }
    }
    ...
    @IBInspectable
    var thirdColor: UIColor = .systemRed {
        didSet {
            updatePaths()
        }
    }
    ...
    func configure() {
        gradientLayer.colors = [firstColor.cgColor, secondColor.cgColor, thirdColor.cgColor]
        gradientLayer.mask = totalShapeLayer
...
extension GradientSlider {
    ...
    func updatedProgress(for touch: UITouch?, touch type: TouchType) {
        ...
        switch type {
        case .began:
            delegate?.gradientSliderBegan(value: Double(currentValue))
        case .moved:
            delegate?.gradientSliderMoved(value: Double(currentValue))
        case .ended:
            delegate?.gradientSliderEnded(value: Double(currentValue))
        }
    }
}
