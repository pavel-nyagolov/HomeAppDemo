...
@IBDesignable
class SolidSlider: UIControl {
    
    var delegate: SolidSliderDelegate?
...
extension SolidSlider {
    ...
    func updatedProgress(for touch: UITouch?, touch type: TouchType) {
        ...
        switch type {
        case .began:
            delegate?.solidSliderBegan(value: Double(currentValue))
        case .moved:
            delegate?.solidSliderMoved(value: Double(currentValue))
        case .ended:
            delegate?.solidSliderEnded(value: Double(currentValue))
        }
    }
}
